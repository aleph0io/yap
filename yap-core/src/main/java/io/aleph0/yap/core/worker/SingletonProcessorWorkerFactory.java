...
/**
 * 
 * A {@link ProcessorWorkerFactory} that manages a singleton {@link MeasuredProcessorWorker} and
 * tracks its metrics.
 * 
 * <p>
 * This factory will only {@link #newProcessorWorker() create} one worker, for which it will return
 * its given worker. All subsequent attempts to create a worker will throw an
 * {@link IllegalStateException}.
 * 
 * @param <InputT> the type of the worker's input
 * @param <OutputT> the type of the worker's output
 * @param <MetricsT> the type of the worker's metrics
 */
public class SingletonProcessorWorkerFactory<InputT, OutputT, MetricsT>
  ...
  public SingletonProcessorWorkerFactory(
      MeasuredProcessorWorker<InputT, OutputT, MetricsT> worker) {

...
public class FlatMapMessageBodyProcessorWorker<T, U>
    extends AcknowledgingMessageBodyProcessorWorker<T, U> {
  private final Function<T, Collection<U>> mapper;

  public FlatMapMessageBodyProcessorWorker(Acknowledger<Message<T>> acknowledger,
  ...
  protected void doProcessMessageBody(T inputBody, Sink<U> sink) throws Exception {
    final Collection<U> outputBodies = mapper.apply(inputBody);
    for (U outputBody : outputBodies)
      sink.put(outputBody);
  }
}
